...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleAuthenticationException(org.springframework.security.core.AuthenticationException ex) {
        ...
        if (ex instanceof DisabledException || ex.getMessage().toLowerCase().contains("disabled") || (ex.getCause() != null && ex.getCause() instanceof DisabledException)) {
    ...
    public ResponseEntity<Map<String, String>> handleGeneralException(Exception ex) {
        System.out.println("DEBUG: Caught general exception: " + ex.getClass().getName() + " - " + ex.getMessage());
        Map<String, String> error = new HashMap<>();
